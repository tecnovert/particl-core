...
Below are some notes on how to build Particl Core for Windows.

The options known to work for building Particl Core on Windows are:
...
Note that for WSL the Particl Core source path MUST be somewhere in the default mount file system, for
example /usr/src/particl, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\particl`, for example:

    make install DESTDIR=/mnt/c/workspace/particl
