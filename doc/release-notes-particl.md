...
- wallet: Removed a byte order mark from the first word (abaisser) of the French BIP39 wordlist.
  - French mnemonics are now compatible with other BIP39 wallets.
  - French mnemonics generated by earlier releases which contain the word abaisser derive different keys.
    Restore them with the legacy_french_bom option of extkeyimportmaster / extkeygenesisimport,
    or the legacy_french_bom parameter of mnemonic decode. Existing wallets are unaffected.
