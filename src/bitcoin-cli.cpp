...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: particl-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to particld). This changes the RPC endpoint used, e.g. http://127.0.0.1:51735/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
public:
    ...
    const std::string m_help_doc{
        ...
        "> particl-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> particl-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title particl-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "See this help\n"
        "> particl-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the particld server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"particl-cli -help\" for more info.",
    ...
    try {
        ...
        if (gArgs.IsArgSet("-getinfo")) {
            ...
            if (method == "extkeyimportmaster"
                ...
                if (args.size() == 0) {
                    args.resize(2);
                    tfm::format(std::cout, "Please enter a mnemonic or private extkey and press return:\n");
                    std::getline(std::cin, args[0]);

                    tfm::format(std::cout, "Please enter passphrase, leave blank for none:\n");
                    ...
                    if (args.size() > 1 && args[1] == "-stdin") {
                        tfm::format(std::cout, "Please enter a passphrase and press return:\n");
                        std::getline(std::cin, args[1]);
                    }
                }
            }
