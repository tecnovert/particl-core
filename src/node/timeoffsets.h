...
class TimeOffsets
{
    ...
    std::chrono::seconds m_time_offset{0};
...
public:
    ...
    /** Return the cached median of the collected time offset samples. The median is returned
     * as 0 when there are less than 5 samples. */
    std::chrono::seconds GetOffset() const EXCLUSIVE_LOCKS_REQUIRED(!m_mutex);
};
