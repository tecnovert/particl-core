...
void TimeOffsets::Add(std::chrono::seconds offset)
{
    ...
    if (Ticks<std::chrono::seconds>(offset) > DEFAULT_MAX_TIME_ADJUSTMENT) {
        offset = std::chrono::seconds(DEFAULT_MAX_TIME_ADJUSTMENT);
    }
    ...
    // m_time_offset = Median(); Can't lock m_mutex twice.
    if (m_offsets.size() < 5) {
        m_time_offset = std::chrono::seconds(0);
    } else {
        auto sorted_copy = m_offsets;
        std::sort(sorted_copy.begin(), sorted_copy.end());
        m_time_offset = sorted_copy[sorted_copy.size() / 2];
    }
}
...
std::chrono::seconds TimeOffsets::GetOffset() const
{
    LOCK(m_mutex);
