...
bool GetFeeRate(const TransactionInfo &info, CFeeRate &feeRate)
{
    const CTransaction &tx = *info.m_tx;
    CAmount tx_fee = info.m_fee;
    ...
    feeRate = CFeeRate(tx_fee - smsg_fees, info.m_virtual_transaction_size);
    return true;
}
