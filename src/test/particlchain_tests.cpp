...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks, bool fAnonChecks = true);
    ...
    for (auto t : tests) {
        ...
        for (auto ti : t.first) {
            if (ti < 0)  {
                ...
                ai.scriptWitness.stack.emplace_back();
...
BOOST_AUTO_TEST_CASE(mixed_output_types)
{
    ...
    BOOST_REQUIRE(0 == PutVarInt(out_fee->vData, txfee));
    ...
    CTransaction tx_c(txn);
    CValidationState state;
    state.SetStateInfo(GetTime(), nSpendHeight, Params().GetConsensus());
...
BOOST_AUTO_TEST_CASE(taproot)
{
    ...
    bool ret = CheckInputs(tx2, state, inputs, flags, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
    BOOST_CHECK(ret);
    }
    ...
    LOCK(cs_main);
    PrecomputedTransactionData txdata;
    bool ret = CheckInputs(tx2, state, inputs, flags_with_taproot, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
    BOOST_CHECK(!ret);
    }
    ...
    LOCK(cs_main);
    PrecomputedTransactionData txdata;
    bool ret = CheckInputs(tx3, state, inputs, flags_with_taproot, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
    BOOST_CHECK(ret);
    }
    }
    ...
    LOCK(cs_main);
    PrecomputedTransactionData txdata;
    bool ret = CheckInputs(tx5, state, inputs, flags_with_taproot, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
    BOOST_CHECK(!ret);
    ...
    bool ret_without_taproot = CheckInputs(tx5, state, inputs, flags, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
    ...
    LOCK(cs_main);
    PrecomputedTransactionData txdata;
    bool ret = CheckInputs(tx6, state, inputs, flags_with_taproot, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
    BOOST_CHECK(ret);
    }
    }
    ...
    // OP_CHECKSIGADD
    {
        ...
        LOCK(cs_main);
        PrecomputedTransactionData txdata;
        bool ret = CheckInputs(tx8, state, inputs, flags_with_taproot, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
        BOOST_CHECK(!ret);
        BOOST_CHECK(state.GetRejectReason() == "non-mandatory-script-verify-flag (Invalid Schnorr signature)");

        // Should pass without SCRIPT_VERIFY_TAPROOT
        bool ret_without_taproot = CheckInputs(tx8, state, inputs, flags, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
        BOOST_CHECK(ret_without_taproot);
        }
        ...
        LOCK(cs_main);
        PrecomputedTransactionData txdata;
        bool ret = CheckInputs(tx9, state, inputs, flags_with_taproot, /* cacheSigStore */ false, /* cacheFullScriptStore */ false, txdata, nullptr);
        BOOST_CHECK(ret);
        }
    }
}
