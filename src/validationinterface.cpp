...
void ValidationSignals::TransactionAddedToWallet(const std::string &sWalletName, const CTransactionRef& tx) {
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.TransactionAddedToWallet(sWalletName, tx); });
}

void ValidationSignals::NewSecureMessage(const smsg::SecureMessage *psmsg, const uint160 &hash) {
    m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.NewSecureMessage(psmsg, hash); });
}

void ValidationSignals::LeavingIBD() {
