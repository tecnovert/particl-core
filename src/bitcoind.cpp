...
static bool ParseArgs(ArgsManager& args, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see particld -h for a list of options.", argv[i])));
        }
    }
    return true;
}
