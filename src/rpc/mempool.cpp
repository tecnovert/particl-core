    ...
    [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
        {
            ...
            const PackageMempoolAcceptResult package_result = [&] {
                LOCK(::cs_main);
                if (txns.size() > 1) return ProcessNewPackage(chainstate, mempool, txns, /*test_accept=*/true, /*client_maxfeerate=*/{}, ignore_locks);
                return PackageMempoolAcceptResult(txns[0]->GetWitnessHash(),
                       chainman.ProcessTransaction(txns[0], /*test_accept=*/ true, ignore_locks));
            }();
