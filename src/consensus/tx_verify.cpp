...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& nTxFee)
{
            ...
            for (size_t k = 0; k < nInputs; ++k) {
                ...
                for (size_t i = 0; i < nRingSize; ++i) {
                    ...
                    if (0 != GetVarInt(vMI, ofs, (uint64_t&)nIndex, nB)) {
                        return state.DoS(100, false, REJECT_INVALID, "bad-anonin-extract-i");
                    }
                    ofs += nB;
                    if (nIndex <= consensusParams.m_frozen_anon_index) {
                        state.m_spends_frozen_blinded = true;
                        if (!IsWhitelistedAnonOutput(nIndex, state.m_time, consensusParams)) {
    ...
    if (state.m_exploit_fix_2) {
        if (state.m_spends_frozen_blinded && spends_post_fork_blinded) {
            return state.DoS(100, false, REJECT_INVALID, "mixed-frozen-blinded");
        }
        if (state.m_spends_frozen_blinded && max_ring_size > 1) {
            return state.DoS(100, false, REJECT_INVALID, "bad-frozen-ringsize");
        }
    }
    ...
    if (state.m_exploit_fix_2 && state.m_spends_frozen_blinded) {
        if (nRingCTOutputs > 0 || nCTOutputs > 0) {
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-frozen-blinded-out");
        }
        if (spends_tainted_blinded && nPlainValueOut + nTxFee > consensusParams.m_max_tainted_value_out) {
            return state.DoS(100, false, REJECT_INVALID, "bad-txns-frozen-blinded-too-large");
        }
    ...
    if (state.m_clamp_tx_version && nValueIn > 0 && nRingCTOutputs > 0 && nCTOutputs > 0) {
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-plain-in-mixed-out");
    }
        ...
        if (nPlainValueOut > 0) {
            if (state.m_exploit_fix_2 && state.m_spends_frozen_blinded) {
                ...
                if (0 != GetVarInt(vData, 1, nTmp, nb) || vData.size() < 1 + nb + 33 || vData[1 + nb] != DO_MASK) {
                    return state.DoS(100, false, REJECT_INVALID, "bad-txns-frozen-bf");
                }
