...
) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    if (confirmations > 0) {
        ...
        push(entry, "trusted", pwallet->IsTrusted(locked_chain, hash, rtx));
    }
    ...
    for (const auto &record : rtx.vout) {
        ...
        if (record.nType == OUTPUT_CT || record.nType == OUTPUT_RINGCT) {
            ...
            if (show_blinding_factors && have_stx &&
                ...
                push(output, "blindingfactor", blinding_factor.ToString());
            }
        }
        outputs.push_back(output);
    }
...
static UniValue listunspentanon(const JSONRPCRequest &request)
{
...

    {
        ...
        pwallet->AvailableAnonCoins(*locked_chain, vecOutputs, !include_unsafe, &cctl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth, fIncludeImmature);
    }
...
static UniValue listunspentblind(const JSONRPCRequest &request)
{
...

    {
        auto locked_chain = pwallet->chain().lock();
        LOCK(pwallet->cs_wallet);
        pwallet->AvailableBlindedCoins(*locked_chain, vecOutputs, !include_unsafe, &cctl, nMinimumAmount, nMaximumAmount, nMinimumSumAmount, nMaximumCount, nMinDepth, nMaxDepth);
    }
...
static UniValue debugwallet(const JSONRPCRequest &request)
{
    ...
    if (list_frozen_outputs || spend_frozen_output) {
        ...
        for (const auto &v : values) {
            ...
            if (rv != 1) {
                throw JSONRPCError(RPC_WALLET_ERROR, "SubmitMemoryPoolAndRelay failed " + state.GetRejectReason());
            }
