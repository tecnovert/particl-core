...
BOOST_AUTO_TEST_CASE(rct_test)
{
    ...
    CTxDestination dest = pk_to.GetID();
    ...
    for (size_t k = 0; k < nSigInputs; ++k) {
        for (size_t i = 0; i < nSigRingSize; ++i) {
            int64_t anon_index;
            BOOST_REQUIRE(0 == GetVarInt(vMI, ofs, (uint64_t&)anon_index, nb));
            ofs += nb;
    ...
    for (size_t i = 0; i < indices.size(); ++i) {
        ...
        BOOST_REQUIRE(0 == PutVarInt(vMI, indices[use_i]));
    }
    ...
    pwallet->AvailableAnonCoins(*locked_chain, vAvailableCoins, true, &cctl, 100000);
    ...
    for (size_t i = 0; i < nCols; ++i) {
        int64_t nIndex;
        BOOST_REQUIRE(0 == GetVarInt(vMI, ofs, (uint64_t&)nIndex, nB));
        ofs += nB;
        ...
        if (pwallet->GetKey(idk, key)) {
            ...
            if (test_keyimage == ki) {
                ...
                BOOST_REQUIRE(stx.GetBlind(ao.outpoint.n, blinding_factor_prevout.begin()));
            }
        }
    }
    BOOST_REQUIRE(real_column < 5000);
    vpBlinds.push_back(blinding_factor_prevout.begin());
    ...
    std::vector<CTempRecipient> vecSend;
    CTxDestination dest = pk_toA.GetID();
    vecSend.emplace_back(OUTPUT_STANDARD, prevouts_sum, dest);
    vecSend.back().fSubtractFeeFromAmount = true;
    ...
    std::vector<CTempRecipient> vecSend;
    CTxDestination dest = pk_toB.GetID();
    vecSend.emplace_back(OUTPUT_STANDARD, prevouts_sum, dest);
    vecSend.back().fSubtractFeeFromAmount = true;
    ...
    {
    LOCK(cs_main);
    CCoinsViewCache tx_view(pcoinsTip.get());
