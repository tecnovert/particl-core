...
BOOST_AUTO_TEST_CASE(frozen_blinded_test)
{
    ...
    CAmount moneysupply_post_fork;
    {
        LOCK(cs_main);
        moneysupply_post_fork = chainActive.Tip()->nMoneySupply;
    }
    pwallet->GetBalances(balances);
    ...
    pwallet->GetBalances(balances);
    CAmount moneysupply_before_post_fork_to_blinded;
    {
        LOCK(cs_main);
        moneysupply_before_post_fork_to_blinded = chainActive.Tip()->nMoneySupply;
    }
    ...
    pwallet->GetBalances(balances);
    CAmount moneysupply_after_post_fork_to_blinded;
    {
        LOCK(cs_main);
        moneysupply_after_post_fork_to_blinded = chainActive.Tip()->nMoneySupply;
    }
    ...
    CAmount moneysupply_after_post_fork_blind_spends;
    {
        LOCK(cs_main);
        moneysupply_after_post_fork_blind_spends = chainActive.Tip()->nMoneySupply;
    }
