...
void CHDWallet::AddOptions()
{
    ...
    gArgs.AddArg("-treasurydonationpercent=<n>", "Percentage of block reward donated to the treasury fund, overridden by system minimum. (default: 0)", ArgsManager::ALLOW_ANY, OptionsCategory::PART_STAKING);
            ...
            for (const auto &prevout : rtx.vin) {
                ...
                if ((mwi = mapWallet.find(prevout.hash)) != mapWallet.end()) {
                    ...
                    if (prevtx.isConflicted()) {
                        MarkConflicted(prevtx.m_confirm.hashBlock, txhash);
                    }
                }
            }
        }
    ...
    bool fInternal, bool fHardened, bool f256bit, bool fBech32, const char *plabel, OutputType output_type)
{
    ...
    if (plabel) {
        ...
        if (output_type == OutputType::BECH32) {
            SetAddressBook(pwdb, WitnessV0KeyHash(PKHash(pkOut)), plabel, "receive", vPath, false);
        } else
        if (f256bit) {
...
bool CHDWallet::TestMempoolAccept(const CTransactionRef &tx, std::string &sError, CAmount override_max_fee) const {
    ...
    bool accept_result = AcceptToMemoryPool(mempool, state, tx, nullptr, nullptr,
...
bool CHDWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm, CValidationState& state)
{
    ...
    return CommitTransaction(wtx_temp, rtx_unused, state, mapValue, orderForm, false);
}
                                  ...
                                  bool is_record, bool broadcast_tx, CAmount override_max_fee)
{
    ...
    if (fBroadcastTransactions && broadcast_tx) {
        ...
        if (!wtx_broadcast->SubmitMemoryPoolAndRelay(err_string, true, *locked_chain, override_max_fee)) {
...
bool CHDWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, CWalletTx::Status status, const uint256& block_hash, int posInBlock, bool fUpdate)
{
    ...
    const uint256& block_hash, int posInBlock, bool fFlushOnClose)
{
    ...
    bool fUpdated = false;
    bool is_from_me = rtxIn.FlagSet(ORF_FROM);
...
bool CHDWallet::CreateCoinStake(unsigned int nBits, int64_t nTime, int nBlockHeight, int64_t nFees, CMutableTransaction &txNew, CKey &key)
{
    ...
    if (!pTreasuryFundSettings || pTreasuryFundSettings->nMinTreasuryStakePercent <= 0) {
        ...
        if (nBlockHeight % pTreasuryFundSettings->nTreasuryOutputPeriod == 0) {
            ...
            if (0 != PutVarInt(vCfwd, nTreasuryCfwd)) {
