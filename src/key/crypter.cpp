...
bool NarrationCrypter::SetKey(const std::vector<uint8_t> &vchNewKey, const uint8_t *chNewIV)
{
    if (vchNewKey.size() != NARR_CRYPTO_KEY_SIZE) {
        return false;
    }
    return SetKey(&vchNewKey[0], chNewIV);
};

bool NarrationCrypter::SetKey(const uint8_t *chNewKey, const uint8_t *chNewIV)
{
...
bool NarrationCrypter::Encrypt(const uint8_t *chPlaintext, uint32_t nPlain, std::vector<uint8_t> &vchCiphertext)
{
    if (!fKeySet) {
        return false;
    }
...
bool NarrationCrypter::Decrypt(const uint8_t *chCiphertext, uint32_t nCipher, std::vector<uint8_t> &vchPlaintext)
{
    if (!fKeySet) {
        return false;
    }
