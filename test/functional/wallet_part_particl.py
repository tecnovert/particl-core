...
class WalletParticlTest(ParticlTestFramework):
    ...
    def run_test(self):
        ...
        try:
            nodes[1].extkeyimportmaster(decodedRoot, '', False, '', '', 0, {'replaceaccount': True})
        ...
        locked_balances = nodes[2].getlockedbalances()
        assert (locked_balances['trusted_plain'] > 0.0)
        ...
        w_rpc = nodes[0].get_wallet_rpc('new_wallet_with_privkeys')
        ek_list_before = w_rpc.extkey('list', True)
