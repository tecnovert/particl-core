This directory contains integration tests that test particld and its
...
particld and particl-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (particl-util, particl-tx, ...).
...
The P2P and RPC ports used by the particld nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another particld
process running on the system (perhaps from a previous test which hasn't successfully
killed all its particld nodes), then there may be a port conflict which will
...
particld processes running on the system, so should not be used if any non-test
particld processes are being run.**
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and particld `debug.log`s will all be dumped
...
`test_framework.log` and particld `debug.log`s can be combined into a single
...
call methods that interact with the particld nodes-under-test.

If further introspection of the particld instances themselves becomes
